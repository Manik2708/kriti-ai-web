...
export class MessageController implements ControllerRegister {
    ...
    registerRouter(): RouterRegistrar {
        ...
        this.object.addControllerWithoutMiddleware(router, "/get", RequestMethodTypes.POST, this.get)
    ...
    private get = async (req: express.Request, res: express.Response) => {
        try {
           const {user_id, project_id, message_id} = req.body;
           const message = await this.messageService.get(user_id, project_id, message_id);
